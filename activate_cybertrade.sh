#!/usr/bin/env bash
# Activate the cybertrade conda env (or fallback venv)
# Usage: source ./activate_cybertrade.sh

# Try real conda first if conda command works and env exists
if command -v conda >/dev/null 2>&1 && conda env list 2>/dev/null | grep -q "cybertrade"; then
  echo "[*] Activating conda env 'cybertrade' via conda"
  # shellcheck disable=SC1091
  eval "$(conda shell.bash hook 2>/dev/null || echo '')"
  conda activate cybertrade 2>/dev/null && {
...
if [ -d "$HOME/miniconda3/envs/cybertrade/bin" ]; then
  echo "[*] Activating simulated conda env at ~/miniconda3/envs/cybertrade"
...
if [ -d ".venv-cybertrade/bin" ]; then
  echo "[*] Activating local venv .venv-cybertrade (fallback for sandbox)"
  # shellcheck disable=SC1091
...
echo "[!] No conda env 'cybertrade' found. Create it with:"
